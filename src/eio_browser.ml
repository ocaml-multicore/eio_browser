...
module Scheduler = struct
  type job = Runnable of (unit -> suspend) | IO
  ...
  and schedule_wakeup t : unit =
    match t.scheduled_wakeup with
    | Some _ -> ()
    | None ->
  ...
  let v run_q = { run_q; io_queued = false; scheduled_wakeup = None }
...
type _ Effect.t +=
  | Enter : (Scheduler.t -> 'a Suspended.t -> suspend) -> 'a Effect.t
...
let next_event : 'a Brr.Ev.type' -> Brr.Ev.target -> 'a Brr.Ev.t =
 fun typ target ->
...
let global_switch_key : Eio.Switch.t Eio.Fiber.key = Eio.Fiber.create_key ()
let global_switch () = Option.get (Eio.Fiber.get global_switch_key)
    ...
    fork ~new_fiber (fun () ->
        r
          ( Eio.Switch.run @@ fun sw ->
            Eio.Fiber.with_binding global_switch_key sw main ))
  in
  result
