...
module Fibers = struct
  ...
  let test_simple_cancel () =
    ...
    let res = Fiber.first (fun () -> "a") (fun () -> Promise.await p) in
...
module Promises = struct
  ...
  let tests = [ Alcotest.test_case "yielding" `Quick test_promises ]
end
...
module Stream = struct
  ...
  let test_stream () =
    ...
    let add s v =
      ...
      l := `Add v :: !l
    in
    let take s = l := `Take (Stream.take s) :: !l in
    ...
    let expected =
      [ `Add 1; `Add 2; `Add 3; `Take 1; `Take 2; `Take 3; `Take 4; `Add 4 ]
    in
    Alcotest.(check (list op)) "same sequence" expected actual

  let tests = [ Alcotest.test_case "stream1" `Quick test_stream ]
end
